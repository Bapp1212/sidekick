...
export async function deleteQuestion(
    ...
    if (questionSnap.data().authorId !== currentUserId)
    ...
    try {
        ...
        await Promise.all(
            answersSnap.docs.map((d) => deleteDoc(d.ref).catch(() => {})),
        );
    } catch (e) {
...
export async function deleteAnswer(
    ...
    if (answerSnap.data().authorId !== currentUserId)
...
export async function createWalkRequest(
    ...
    const leavingText =
        leavingInMins === 0 ? "now" : `in ${leavingInMins} mins`;
...
export async function updateWalkRequestLocation(
    requestId: string,
    lat: number,
    lng: number,
) {
...
export async function updateLocation(
    ...
    const session = sessionSnap.data();
    const locationField =
        userId === session.userAId ? "userALocation" : "userBLocation";
